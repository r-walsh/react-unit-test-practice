...
describe('NewToDo', () => {
  ...
  it('renders a list of to dos based on state.toDos', () => {
    const list = shallow(<List/>);
    list.setState({ toDos: [{ complete: false, text: 'One' }, { complete: true, text: 'Two'}]})
  ...
  it('updates state.toDos on submitToDo', () => {
    ...
    expect(list.state().toDos).toEqual([{complete: false, text: 'A new to do'}]);
  })
  ...
  it('toggles a toDo to complete on toggleCompletion', () => {
    ...
    expect(list.state().toDos).toEqual([{ complete: true, text: 'One' }, { complete: true, text: 'Two'}])
  })
});
